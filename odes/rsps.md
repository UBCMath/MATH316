...
$$
\begin{equation}
\lim\limits_{n\ra\infty}\frac{c_{m+2}}{c_m}=\frac{m+3}{m+1}=1\quad
\rho =1
\end{equation}
$$(ref7)
...
y=x^r\ra r(r-1)+r=0\ra y=c_1+c_2\ln x\end{array}
$$
...
y=x^r\ra r(r-1)-2=0\quad r=2,-1\quad
...
y=x^r\ra r(r-1)-2r+2=0\quad r=1,2\quad y=c_1x+c_2x^2\end{array}
$$
...
$$
\begin{equation}
x^2 y^{\prime\prime}+\al {x}y^\prime +\beta y=0.
\end{equation}
$$(ref10)
...
$$
\begin{equation}
\lim\limits_{x\ra x_0}(x-x_0)\frac{Q(x)}{P(x)}\quad\mbox{and}\quad
\lim\limits_{x\ra x_0}(x-x_0)^2\frac{R(x)}{P(x)}\quad\mbox{are
finite.}
\end{equation}
$$(ref11)
...
& &\quad\quad \ra \mbox{ singularity no worse than $\frac{1}{x-x_0}$}\\
(x-x_0)^2 \frac{R(x)}{P(x)} & = &
q_0+q_1 (x-x_0)+q_2(x-x_0)^2+\cdots\nonumber\\
& &\quad\quad \ra \mbox{ singularity no worse than
$\frac{1}{(x-x_0)^2}$}\nonumber
\end{eqnarray}
$$(ref12)
...
\lim\limits_{x\ra
1}(x-1)\frac{(-2x)}{(1-x)(1+x)}=1\quad\lim\limits_{x\ra
...
\lim\limits_{x\ra 0}x^2\left(\frac{-1}{x^3}\right)
=\infty\end{array}
\end{eqnarray}
$$(ref14)

Thus $x=0$ is an irregular singular point. Actually $y \sim
x^{3/4}\rme^{\pm 2/x^{1/2}}$ as ${x\ra 0+}$ which is much wilder
...
$\dst\left(\frac{xQ}{P}\right)$ and
$\dst\left(\frac{x^2R}{P}\right)$ are analytic at $x=0$, i.e.,
...
p_1xy^\prime +q_1y+\cdots\right\}}^{\mbox{small as }x\ra 0} = 0.
\end{equation}
$$(ref17)

Then as $x\ra 0$ $x^2y^{\prime\prime}+xp_0y^\prime
...
form $\dst y=x^r\sum\limits_{n=0}^\infty
...
````{prf:example}
$$
x^2y^{\prime\prime}+2(\rme^x-1)y^\prime
+\rme^{-x}\cos xy=0,\quad P=x^2$, $Q=2(\rme^x-1)$, $R=\rme^{-x}\cos
x
$$
...
$$
\begin{eqnarray}
\lim\limits_{x\ra 0}\frac{xQ}{P} & = & \lim\limits_{k\ra 0}x
\frac{2(\rme^x-1)}{x^2} = \lim\limits_{x\ra 0}\frac{2(\rme^x-1)}{x}
\stackrel{\frac{0}{0}}{=}\lim\limits_{x\ra 0}\frac{2\rme^x}{1}=2\mbox{ L'Hopital}\nonumber\\
\lim\limits_{x\ra 0}\frac{x^2R}{P} & = & \lim\limits_{x\ra
0}x^2\frac{\rme^{-x}\cos x}{x^2}=1<\infty .
\end{eqnarray}
$$(ref18)
